...
contains
      ...
      r = sqrt(x%x1**2 + x%x2**2 + x%x3**2)
      init = exp(-0.5d0*(r/Sigma)**2)
      ...
      contains
         ...
         r = sqrt(xt%x1**2 + xt%x2**2 + xt%x3**2)
         b1_equation = rMax - r
...
contains
      ...
      coeff%Dij = (/1.d-1, 0.d0, 1.d-1, 0.d0, 0.d0, 1.d-1/)
      coeff%djDij = (/0.d0, 0.d0, 0.d0/)
      coeff%hi = (/0.d0, 0.d0, 0.d0/)
      coeff%dihi = 0.d0
      coeff%S = 2.5d0
      coeff%v = 1.d-6/(1.d0 + sqrt(xt%x1**2 + xt%x2**2 + xt%x3**2))
