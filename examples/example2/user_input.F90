...
contains
      ...
      real(8), parameter :: sigma = 5.d-2
      associate( x1=>x%x1 )

      init = exp(-(x1 - mu)**2/(2.d0*sigma**2))
...
!      use mod_grid
!      use functions
      use mod_solver, only: solution_prev
      implicit none
...
contains
      ...
      coeff%Dij = (/1.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0/)
      ...
      if( xt%x1<0.5d0 ) then
         ...
         coeff%S = 0.d0
         x_tilde = xt%space
         x_tilde%x1 = xt%x1 - 0.5d0
         call solution_prev%get(x_tilde, f_tilde)
         coeff%v = 1.d1*f_tilde
      end if
